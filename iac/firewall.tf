locals {
  nodesToBeProtected = concat([ for node in linode_lke_cluster.controller.pool[0].nodes : node.instance_id ], flatten([ for worker in linode_lke_cluster.workers : [ for node in worker.pool[0].nodes : node.instance_id ]]))
  allowedPublicIps   = concat([ for node in data.linode_instances.clustersNodes.instances : "${node.ip_address}/32" ], [ "${jsondecode(data.http.myIp.response_body).ip}/32" ])
  allowedPrivateIps  = [ for node in data.linode_instances.clustersNodes.instances : "${node.private_ip_address}/32" ]
